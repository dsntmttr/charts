...
elements
than kids overwrite e.g fail object

class prep data for chart
e.g
includes pollohlcdata and min max calc 

class chart control
e.g
includes switching css classes, controling curret actives etc.
updating actives as a method
...
data object http://softwareengineering.stackexchange.com/questions/237552/how-can-i-keep-web-services-requests-in-a-dao-layer-without-tying-the-code-to-th
controller object
service object view object etc
http://crushlovely.com/journal/7-patterns-to-refactor-javascript-applications-view-objects/
...
function dataForChart(){
	// problem with improting data from parent function which needs data inserted
	this.ohlcArr = [];
	this.volumesArr = [];
	this.lowsArr = [];
	this.highsArr = [];
	...
	this.predDataArrays = function(data){
		for(i in data) {
	        this.ohlcArr.push([(data[i].time) * 1000, Number(data[i].open), Number(data[i].high), Number(data[i].low), Number(data[i].close)]);
	        this.volumesArr.push([(data[i].time) * 1000, Number(data[i].vol)]);
	        this.highsArr.push(Number(data[i].high));
	        this.lowsArr.push(Number(data[i].low));
	        this.datesArr.push((data[i].time) * 1000);
	    }
	}    
}
function plotExtremes(){
	var min,
		max;
}
